...
class DataExtraction():
    def __init__(self, config: DataIngestionConfig, logger: logging.Logger=logger) -> None:
        self.helper = Helper()
        self.config = config
        self.logger = logger
    ...
    def extract_data(self) -> None:
        try:
            ...
            raw_local_file_path = Path(os.path.join(self.config.raw_file_dir, self.config.raw_file_name))
            self.helper.save_dataframe(df, raw_local_file_path, self.logger, index=False)
        except Exception as e:
            self.logger.error(CustomException(e, sys))
            raise CustomException(e, sys) from e
