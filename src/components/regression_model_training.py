        ...
        try:
            ...
            for model in self.params.models:
                ...
                with mlflow.start_run() as run:
                    ...
                    self.logger.info(f'mlflow_model_uri: {model_uri}')
                    model_params = self.params.models[model]
                    self.logger.info("------------")
                    self.logger.info(model)
                    self.logger.info("------------")
                    study = optuna.create_study(
                        direction=self.optimize_direction, sampler=TPESampler()
                    )
                    study.optimize(
                        lambda trial: TrainModel.objective(
                            ...
                            X_train,
                            y_train,
                            scoring=self.optimize_scoring,
                        ),
                        n_trials=self.params.optuna_trials,
                        show_progress_bar=True,
                    )
                    self.logger.info(
                        f" Best parameters for {model} is : {str(study.best_params)}"
                    )
                    ...
                    evaluation_metrics = self._evaluate(ml_model, X_test, y_test)
                    metrics_dict = {
                        "Train score": study.best_value,
                        **evaluation_metrics,
                    }
                    # print(metrics_dict)
                ...
                model_version_infos = client.search_model_versions(
                    "name = '%s'" % self.mlflow_model_registry_name
                )
        ...
        for suggest_type in model_params.params:
            if suggest_type == "int":
                for arg in model_params.params[suggest_type]:
                    params[arg] = trial.suggest_int(
                        arg, *model_params.params[suggest_type][arg]
                    )
            elif suggest_type == "float":
                for arg in model_params.params[suggest_type]:
                    params[arg] = trial.suggest_float(
                        arg, *model_params.params[suggest_type][arg]
                    )
            elif suggest_type == "float_log":
                for arg in model_params.params[suggest_type]:
                    params[arg] = trial.suggest_float(
                        arg, *model_params.params[suggest_type][arg], log=True
                    )
            elif suggest_type == "categorical":
                for arg in model_params.params[suggest_type]:
                    params[arg] = trial.suggest_categorical(
                        arg, model_params.params[suggest_type][arg]
                    )

        model = TrainModel._prepare_model(
            module_name=model_params.module, class_name=model_name, params=params
        )
        metrics = -np.mean(
            cross_val_score(model, X_train, y_train, cv=4, n_jobs=-1, scoring=scoring)
        )
        return metrics
